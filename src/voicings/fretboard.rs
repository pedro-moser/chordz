...
impl Fretboard {
    ...
    pub fn get_note(&self, string: usize, fret: usize) -> Note {
        self.open_strings[string].transpose(fret as i32)
    }
...
mod tests {
    ...
    fn test_standard_tuning_open_strings() {
        ...
        assert_eq!(fb.get_note(0, 0).pc_name(), "E");
        assert_eq!(fb.get_note(1, 0).pc_name(), "A");
        assert_eq!(fb.get_note(2, 0).pc_name(), "D");
        assert_eq!(fb.get_note(3, 0).pc_name(), "G");
        assert_eq!(fb.get_note(4, 0).pc_name(), "B");
        assert_eq!(fb.get_note(5, 0).pc_name(), "E");
    }
    ...
    fn test_fret_transposition() {
        let fb = Fretboard::standard_tuning();
        // 5th fret on low E = A
        assert_eq!(fb.get_note(0, 5).pc_name(), "A");
        // 7th fret on A = E (perfect 5th)
        assert_eq!(fb.get_note(1, 7).pc_name(), "E");
        // 12th fret is octave of open string
        assert_eq!(fb.get_note(0, 12).pitch_class, fb.get_note(0, 0).pitch_class);
    }
}
